...
去调用它！！）。**自己注：是不是意思是说一个`Verticle`实例会绑定一个固定的事件轮询上？？？**

**这意味着你可以在你的应用程序中以单线程的方式写所有代码，不再同步的担心，避免了许多竞争条件和死锁，特别是
传统多线程应用程序开发如此盛行的现在**。

### Worker verticles
Worker verticles挺像标准Verticles，但它 **不是由事件轮询执行**，而是由vert.x的 **worker线程池** 中的线程执行。
...
```
你也能通过指定 **verticle的名字** 来部署一个Verticle。verticle名字是用来查找指定的`VerticleFactory`，
...
能有一个前缀（这是一个字符串），后跟随一个 冒号，目前用于查找Verticle工厂**。例如：
```
...
```
如果部署成功，**处理完成执行程序** 将提供一个包含 **部署ID字符串** 的结果。如果你想取消部署，这个部署ID后面还能用到。
...
### 配置Verticle
部署时，可以传递一个 **JSON** 对象的配置给Verticle：
```java
...
```
### 在Verticle里访问环境变量
**环境变量** 与 **系统属性** 是通过JAVA API可以访问的：
```java
System.getProperty("prop");
System.getenv("HOME");
```
### Verticle组隔离
默认情况下，vert.x有一个扁平的`classpath`。即，vert.x使用 **当前类加载器** 部署Verticle，根本不用再
...
当使用一个隔离组时，你要提供一个你想隔离的 **类名列表** 给`setIsolatedClasses`方法（一个 **全限定类名**，就像
...
```
### 高可用（HA）
`Verticles`能启用HA（High Availability）方案部署。在这种情况下，当一个`verticle`在一个vert.x
实例上部署时，如果突然崩溃，`verticle`会在集群的另一个vert.x实例上重新部署。

启用HA方案运行`verticle`，仅仅需要添加`-ha`，示例：
```powershell
$ vertx run my-verticle.js -ha
```
当启用HA方案时，不需要添加`-cluster`。更多有关HA方案的细节请看：**HA与热备** 小节。

### 从命令行运行Verticles
略了吧！！！！！！！！！

### 让Vert.x退出
由vert.x实例保持并管理的线程 **不是守护线程**，所以它们将防止vert.x从JVM中退出。如果你内嵌vert.x
并且已经完成了，你能调用`close`方法来关闭它，这将 **关闭所有内部线程池** 并 **释放所有资源**，然后退出JVM。

### 上下文对象
Vert.x提供一个事件到处理程序或者调用`Verticle`的`start`或`stop`方法时，这个执行是与`Context`想关的。
通常`Context`指的是一个事件轮询上下文，并与特定的事件轮询线程联系在一起。因此上下文的执行总是发生在
相同的事件循环线程上。以`worker verticle`为例，一个`worker Context`将被关联到使用worker线程池中的一个线程进行的执行上。

要获得上下文，使用`getOrCreateContext()`方法：
```java
Context context = vertx.getOrCreateContext();
```
如果当前线程有一个上下文与之关联，它会 **重用** 这个上下文对象；如果没有，它将 **创建** 一个新的上下文实例。
你可以测试你获取的上下文类型：
```java
Context context = vertx.getOrCreateContext();
...
```
当你检索上下文对象时，能在这个上下文中异步运行代码。换句话说，你提交一个任务，最终会运行在相同的上下文中。
```java
vertx.getOrCreateContext().runOnContext((v) -> {
  System.out.println("This will be executed asynchronously in the same context");
});
```
当几个处理程序在相同的上下文中运行时，他们可能希望共享数据。上下文对象提供了在上下文中存储和检索共享
数据的方法。例如，它可以让你传递数据到运行`runOnContext`的一些行为：
```java
