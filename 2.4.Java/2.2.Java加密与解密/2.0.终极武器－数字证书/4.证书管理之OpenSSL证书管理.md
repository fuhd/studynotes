...
$ mkdir certs
$ echo 构建新证书存放目录 demoCA/newcerts
...
```shell
# 构建相关文件
$ echo 构建索引文件 demoCA/index.txt
$ echo 0 > demoCA/index.txt
$ echo 构建序列号文件 demoCA/serial
...
openssl x509 -req -days 3650 -sha1 -extensions v3_req -CA certs/ca.cer 
	-CAkey private/ca.key.pem -CAserial ca.srl -CAcreateserial 
...
这里我们同样需要将`OpenSSL`产生的数字证书转化为`PKCS#12`编码格式。完整的命令如下：
```shell
#服务器证书转换
echo 服务器证书转换 private/server.p12
openssl pkcs12 -export -clcerts -inkey private/server.key.pem 
...
现在，我们已经构建了服务器证书（`server.cer`），并可使用该证书构建基于 **单向认证** 的网络交易平台。

### 构建客户证书
**客户证书的构建与服务器证书构建基本一致，首先需要构建私钥**。完整命令如下：
```shell
#产生客户私钥
...
这里的参数`2048`，指`RSA`密钥长度位数，默认长度为`512`位。这时我们需要输入服务器证书密码，如“123456”。

完成客户证书密钥构建后，我们需要产生 **客户证书签发申请**。完整命令如下：
```shell
#生成客户证书签发申请
echo 生成客户证书签发申请 client.csr
openssl req -new -key private/client.key.pem -out private/client.csr 
	-subj "/C=CN/ST=BJ/L=BJ/O=zlex/OU=zlex/CN=zlex"
```
各参数的含义如下：
...
我们已经获得了 **根证书**，可以使用根证书 **签发客户证书**（`client.cer`）。完整命令如下：
```shell
#签发客户证书
echo 签发客户证书 certs/client.cer
openssl ca -days 3650 -in private/client.csr -out certs/client.cer -cert certs/ca.cer 
...
这时我们需要输入服务器证书密码，如“123456”，并同意签发证书。
```
执行后，出现以下错误：
...
最后，我们需要 **将获得客户证书转化`Java`语言可以识别的`PKCS#12`编码格式**。完整命令如下：
```shell
#客户证书转换
echo 客户证书转换 certs/client.p12
openssl pkcs12 -export -inkey private/client.key.pem -in certs/client.cer 
