...
**用户看不到以下属性**。根据作业类型实现，这些属性用于约束用户作业和属性。这些是在
`AbstractProcessJob.java`中合并形成`sysProps`的相同属性：

| PropertySource | 描述 | 优先级 |
|:------------- |:------ |:----- |
| `commonprivate.properties`在`jobtype`目录中 | Azkaban设置期间的管理员配置属性。全局所有工作类型。 | 最低（0） |
| `private.properties`在`jobtype/{jobtype名}`目录 | Azkaban设置期间的管理员配置属性。受限于特定的工作类型。| 最高（1） |

`azkaban.properties`是另一种类型的属性，仅用于控制`Azkaban webserver`和`execserver`配置。
请注意`jobProps`，`sysProps`并且`azkaban.properties`是3种不同类型的属性，并且一般不合并（
取决于作业类型实现）。
