...
### 3.1.Azkaban Jobtype插件配置
这些属性用于配置随AzkabanExecutorServer一起安装的作业类型插件。请注意，Azkaban使用目录结构来
推断全局设置与单个作业类型特定设置。子目录名称还确定运行Azkaban实例的作业类型名称。

#### 3.1.1.介绍
Jobtype插件确定各个作业在本地或远程群集上的实际运行方式。它带来了很多好处：可以在不触及Azkaban
核心代码的情况下添加或更改任何作业类型; 一个人可以轻松地扩展Azkaban在不同的hadoop版本或发行版
上运行; 在添加相同类型的新版本时，可以保留旧版本。但是，管理这些插件的管理员确实需要确保安装和配
置正确。

在AzkabanExecutorServer启动后，Azkaban将尝试加载它可以找到的所有作业类型插件。Azkaban将做
非常简单的测试并丢弃坏的测试。应该总是尝试运行一些测试作业，以确保作业类型真正按预期工作。

#### 3.1.2.全局属性
可以将全局设置传递给所有作业类型，包括将由所有作业类型使用的群集相关设置。这些设置也可以在每个作业
类型的自己的设置中指定。

#### 3.1.3.私有设置
可以传递作业类型所需的全局设置，但不能通过`commonprivate.properties`中的用户代码访问。例如，
hadoop集群通常需要以下设置：

| 参数 | 描述 |
|:-----|:----|
| `hadoop.security.manager.class` | hadoop安全管理器处理与hadoop cluseter的谈话。 对于1.x版本，请使用`azkaban.security.Hadoop Security Manager_H_1_0` |
| `azkaban.should.proxy` | 无论Azkaban是代理个人用户hadoop帐户，还是作为Azkaban用户自行运行，默认为`true` |
| `proxy.user` | Azkaban用户配置了kerberos和hadoop。 类似于应如何配置oozie，用于安全的hadoop安装 |
| `proxy.keytab.location` | 密钥表文件的位置，Azkaban可以使用Kerberos为指定的proxy.user进行身份验证 |
| `jobtype.global.classpath` | 每个作业类型应该在其类路径上具有的jar或xml资源。（例如：`${hadoop.home}/hadoop-core-1.0.4.jar, ${hadoop.home}/conf`） |
| `jobtype.global.jvm.args` | 每个工作类型应该具有jvm的jvm args。|
| `hadoop.home` | `$HADOOP_HOME`设置。|

#### 3.1.4.公共设置
可以传递作业类型所需的全局设置，并且可以通过用户代码在`common.properties`中查看。例如，`hadoop.home`
通常应该传递给用户程序。

#### 3.1.5.各种作业类型的设置
在大多数情况下，除了像`hadoop.home`，`pig.home`，`hive.home`等变量之外，工作类型不需要额外
的设置。但是，它也是大多数自定义的来源。例如，可以使用相同的jar资源配置两个Java作业类型，但具有
不同的hadoop配置，从而将pig作业提交到不同的集群。还可以为特定组织配置预先注册的jar和名称空间导
入的pig作业。另外需要注意的是：在常见作业类型插件列表中，我们包含了不同的pig版本。管理员需要与其
中一个建立软链接，例如：
```shell
$ ln -s pig-0.10.1 pig
```
