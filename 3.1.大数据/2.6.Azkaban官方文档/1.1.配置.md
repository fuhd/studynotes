...
### 1.执行用户
通过`Azkaban 3.0`中的新安全增强功能，Azkaban作业现在可以作为提交用户或默认情况下的用户的
`user.to.proxy`运行。这确保了Azkaban利用Linux权限安全机制，并且在操作上这简化了资源监视和可
见性。通过执行以下操作设置此行为：

默认情况下，`Execute.as.user`设置为true。如果需要，它也可以在azkaban-plugin的
`commonprivate.properties`中配置为false。将`azkaban.native.lib`配置到您要放置已编译的
`execute-as-user.c`文件的位置（见下文）生成一个Azkaban-box上的可执行文件，用于
`azkaban-common/src/main/c/execute-as-user.c`。它应该被命名为`execute-as-user`下面是
一个示例方法。
+ `scp ./azkaban-common/src/main/c/execute-as-user.c`到Azkaban box
+ 运行: `gcc execute-as-user.c -o execute-as-user`
+ 运行: `chown root execute-as-user`(您可能需要root权限)
+ 运行: `chmod 6050 execute-as-user`(您可能需要root权限)
