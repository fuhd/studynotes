...
**第四步让hadoop用户组的用户对该目录有写的权限**
```shell
$ sudo chmod 775 /var/log/hadoop/
```
...
$ chmod -R 775 hadoop_data/dfs/yarn_local_dir
```
编辑yarn-site.xml文件：
```xml
<?xml version="1.0"?>
...
### 10.在$HADOOP_CONF_DIR下新增配置mapred-site.xml
...
按原计划把辅助namenode（secondarynamenode）配置到server1（hadoop1）上。
