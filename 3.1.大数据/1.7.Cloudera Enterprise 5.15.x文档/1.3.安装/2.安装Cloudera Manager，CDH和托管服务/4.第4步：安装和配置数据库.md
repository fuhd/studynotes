...
+ Cloudera Manager服务
+ Cloudera管理服务角色：
  - 活动监视器（如果在CDH5群集中使用MapReduce服务）
  - 报告管理器
+ 每个Hive metastore
+ Sentry服务
+ Cloudera Navigator审计服务
+ Cloudera Navigator元数据服务

数据库必须配置为支持MySQL **utf8** 字符集编码。

记录您为 **数据库名称，用户名和密码** 输入的值。Cloudera Manager安装向导需要此信息才能正确连接到
这些数据库。
1. 以root用户或具有创建数据库和授予权限的其他用户身份登录：
    ```shell
    $ mysql -u root -p
    ```
2. 从下表中为您正在使用的每个服务创建数据库：
    ```sql
    MariaDB [(none)]> CREATE DATABASE <database> DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;
    Query OK, 1 row affected (0.00 sec)

    MariaDB [(none)]> GRANT ALL ON <database>.* TO '<user>'@'%' IDENTIFIED BY '<password>';
    Query OK, 0 rows affected (0.00 sec)
    ```
    您可以使用<database>，<user>和<password>的任何值。以下示例是 **Cloudera Manager配置设置中
    提供的默认名称，但您不需要使用它们**：
