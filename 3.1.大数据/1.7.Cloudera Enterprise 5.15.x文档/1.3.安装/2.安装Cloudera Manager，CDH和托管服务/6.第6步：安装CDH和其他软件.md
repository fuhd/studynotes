...
1. 选择用于安装的存储库类型。在 **选择方法** 部分中，选择以下选项之一：
    + 使用Parcels：
        ...
        默认情况下只配置最近支持的服务版本的parcels。

        您可以通过指定自定义存储库为以前的版本添加其他parcels。例如，您可以在
        https://archive.cloudera.com/cdh4/parcels/ 找到以前的CDH4 parcels的位置。或者，
        如果您要安装CDH4.3并希望使用策略文件授权，则可以使用此机制添加Sentry parcels。
