...
1. 选择用于安装的存储库类型。在 **选择方法** 部分中，选择以下选项之一：
    + 使用Parcels：
        ...
        添加Sentry parcels。
          ```
          注：最新的CDH5.15.x可以在这里找到： https://archive.cloudera.com/cdh5/parcels/
          ```
...
2. 选择一种认证方法：
    ...
    如有必要，您可以修改默认的SSH端口。
3. 指定一次运行的主机安装的最大数量。**默认值和推荐值为10**，您可以根据网络容量调整此值。
4. 点击 **继续**。

显示“安装代理”页面。

## 8.安装代理
“安装代理”页面显示安装进度。您可以点击任何主机的详细信息链接查看安装日志。如果安装失败，您可以单击
“中止安装”按钮取消安装，然后查看安装日志以解决问题。

如果安装在任何主机上都失败，您可以单击“重试失败的主机”重试所有失败的主机，也可以单击特定主机上的“重
试”链接。

在所有主机上安装Cloudera Manager Agent后，单击继续。

如果您正在使用parcels，则显示安装parcels页面。如果您选择使用packages进行安装，则会显示“检查主机”
页面。

## 9.安装Parcels
如果您选择Parcels作为安装方法，则“安装Parcels”页面会报告您之前选择的Parcels的安装进度。下载Parcel后，
出现进度条表示每个群集主机。您可以单击单个进度栏以获取有关该主机的详细信息。

安装完成后，请点击继续，检查主机页面显示。

## 10.检查主机
检查主机页面运行主机检查器来搜索常见的配置问题。查看结果并解决任何已识别的问题。点击“再次运行”按钮更
改结果后进行更改。
