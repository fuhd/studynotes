...
object Chapter3_3_2 {
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf()
      .setMaster("yarn-cluster") //设置目标Master通信地址
      .setAppName("WC") //设置任务名称
    //实例化SparkContext,Spark的对外接口负责用户与Spark内部的交互通信
    val sc = new SparkContext(conf)
    //读取文件并进行单词统计
    sc.textFile("hdfs:/ns/user/maxcompute/users/admin/text/abc.txt")
      .flatMap(_.split(" "))
      .map((_, 1))
      .reduceByKey(_ + _, 1)
      .saveAsTextFile("hdfs:/ns/user/maxcompute/users/admin/text/abc_result")
    sc.stop()
  }
}
```
