...
```
个人操作反馈：
目前，必须在etc/hadoop/hadoop-env.sh中添加JAVA_HOME环境变量才可以使用，要不然根本运行不起来。
```
...
<configuration>
    ...
    <!-- 这是一个临时目录，经常造成 http://localhost:50070 访问不了（默认在tmp目录下，会定期清理），
    这里显示设置它（这个目录要提前准备好） -->
    <property>
...
<configuration>
    ...
    <!-- core-site.xml中加了hadoop.tmp.dir，而hdfs-site.xml中不加这个配置，
    SecondaryNameNode会报Inconsistent checkpoint fields异常 -->
    <property>
        <name>hadoop.tmp.dir</name>
        <value>/opt/hadoop/tmp</value>
        <description>A base for other temporary directories.</description>
...
来检查守护进程是否成功启动，或通过`Web`界面：在 **`http://localhost:50070`** 查看`namenode`、在 **`http://localhost:8088/`**
