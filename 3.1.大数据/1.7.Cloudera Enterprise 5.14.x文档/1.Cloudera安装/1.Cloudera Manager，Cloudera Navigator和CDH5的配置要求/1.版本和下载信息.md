...
为了避免这种情况，请将Maven依赖关系范围设置为provided。 如果您已经构建了包含CDH JAR的应用程序，请更新依赖项以将范围设置
为provided和recompile。
```
以下是示例POM（**pom.xml**）文件：
```xml
...
### 3.Cloudera Navigator Key Trustee服务器（密钥托管服务器？）版本和下载信息
您可以在Cloudera Manager中使用parcels或使用软件包来安装Key Trustee Server。

#### Navigator Key Trustee Server 5.14
Navigator Key Trustee Server 5.14.0（RHEL, CentOS, and Oracle Linux）：
+ Package库tarball：转到Cloudera Navigator Key Trustee Server [下载页面](https://www.cloudera.com/downloads/navigator/key-trustee-server/5-14-0.html)。
从Package或Parcel下拉菜单中选择Packages，选择您的操作系统版本，然后单击立即下载。
+ Package安装命令：**yum install keytrustee-server-5.14.0**
+ Parcel库tarball：转到Cloudera Navigator Key Trustee Server [下载页面](https://www.cloudera.com/downloads/navigator/key-trustee-server/5-14-0.html)。
从Package或Parcel下拉菜单中选择Parcels，选择您的操作系统版本，然后单击立即下载。
