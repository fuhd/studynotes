...
```
或者，你可以使用以下命令启动sqoop job并 **观察job运行状态**：
```shell
sqoop:000> start job -n job_case_cf_baseinfo -s
```
最后，你可以使用停止job命令随时 **停止运行job**：
```shell
sqoop:000> stop job -n job_case_cf_baseinfo
```
