...
1. 在Review页面上，仔细查看配置。如果一切正常，请单击“部署”以在Ambari服务器上安装Atlas。
    ...
    如果启用了Kerberos，系统将提示您输入KDC管理员凭据。键入您的KDC管理员主体和密码，然后单击
    “保存”。

    ![kerberos](img/13.png)

2. 单击“部署”时，Atlas将安装在Ambari服务器上的指定主机上。进度条显示安装进度。

    ![部署](img/14.png)

3. 安装完成后，“摘要”页面将显示安装详细信息。单击“完成”以完成安装。
    ...
    默认情况下，Atlas用户名和密码设置为admin / admin。
    ```

4. 选择`Actions > Restart All Required`以重新启动需要重新启动的所有群集组件。
