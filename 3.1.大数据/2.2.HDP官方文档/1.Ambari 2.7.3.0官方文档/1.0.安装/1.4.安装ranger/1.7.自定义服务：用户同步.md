...
+ 自定义LDAP/AD的用户同步：
    ...
    6. 在“组配置”选项卡上设置以下属性。
        ...
        | Enable Group Search First | 选择“启用组搜索优先”后，有两种可能的方法来检索用户：1.如果未选择“启用用户搜索”：将从组的“成员”属性中检索用户。2.如果选择了“启用用户搜索”：通过基于用户配置执行LDAP搜索来计算用户成员资格。 | NO | YES |
        | state:anchor=nested_ldapad_syncSyncNested Groups | 在Ranger中启用嵌套组成员身份，以便为父组配置的策略应用于子组中的所有成员的嵌套组。如果组本身是另一个组的成员，则属于该成员组的用户也是该父组的一部分。组层次结构级别确定评估的嵌套组深度。如果没有看到“同步嵌套组”标志，请升级到Ambari 2.6.0+。 | NO | YES,NO |
        | Group Hierarchy Levels | 确定要评估的嵌套组的数量，以支持同步嵌套组。如果“组层次结构级别”显示为灰色，请启用“同步嵌套组”。设置为任何大于0的整数。 | 0 | 2 |
