...
默认情况下，Ambari为Hive Metastore安装新的MySQL实例，并为Oozie安装Derby实例。如果您计划将使用现有数据库：
MySQL/MariaDB，Oracle或PostgreSQL，继续之前请修改数据库类型和主机。有关在MariaDB上创建外部数据库的快速示例，请参阅管
理Ambari中的示例：安装MariaDB以与多个组件一起使用。
```
```
重要
...
### 目录
为数据和日志存储选择正确的目录至关重要。Ambari根据您环境中可用的挂载点选择合理的默认值，但强烈建
议您查看Ambari推荐的默认目录设置。特别是，确认目录（如`/tmp`和`/var`）不用于HDFS选项卡下的
HDFS NameNode目录和DataNode目录。

### 帐号
服务帐户用户和组也可以从“帐户”选项卡进行配置。这些是服务组件将运行的操作系统帐户。如果主机上不存
在这些用户，Ambari将自动在主机上本地创建用户和组。如果这些用户已存在，Ambari将使用这些帐户。

根据环境的配置方式，您可能不允许使用`groupmod`或`usermod`操作。如果是这种情况，可以选择多种方
式来选择Ambari应如何处理用户创建和修改：

| 操作 | 描述 |
| :------------- | :------------- |
| 使用Ambari管理服务帐户和组 | 如果每个服务在 `/etc/password` 中以及Ambari托管主机的 `/etc/group`中不存在，Ambari将创建所需的服务帐户和组。|
| 使用Ambari管理组成员身份 | Ambari将添加或删除组中的服务帐户。 |
| 使用Ambari管理服务帐户UID | Ambari将能够更改所有服务的UID账户。 |

### 所有配置
在这里，您有机会查看和修改服务的其余配置。浏览每个配置选项卡。将光标悬停在每个属性上，会显示属性
的简要说明。此处显示的服务选项卡数取决于您决定在群集中安装的服务。任何需要注意的配置问题的服务都
会显示在带有需要注意的数字属性的铃声图标中。

![配置](img/10.png)

响铃弹出包含需要您注意的配置，强烈建议检查和更改的配置，以及将根据Ambari的建议自动更改的配置，除
非您选择退出这些更改。必须解决必需的配置才能继续执行向导中的下一步。在继续之前，请仔细阅读所需和
建议的设置并解决问题。

完成自定义服务后，选择“下一步”。

## 12.Review
*Review* 显示您所做的分配。检查以确保一切正确。如果需要进行更改，请使用左侧导航栏返回相应的屏幕。

要打印您的信息以供日后参考，请选择“*打印*”。
...
## 13.安装、启动和测试
安装进度显示在屏幕上。Ambari在每个组件上安装，启动和运行一个简单的测试。进程的总体状态显示在屏幕
顶部的进度条中，主体中显示主机状态。在此过程中不要刷新浏览器。**刷新浏览器可能会中断进度指示器**。

要查看有关每个主机已完成的任务的特定信息，请单击相应主机的“消息”列中的链接。在“任务”弹出窗口中，
单击单个任务以查看相关的日志文件。您可以使用“显示”下拉列表选择过滤条件。要查看更大版本的日志内容，
请单击“打开”或将内容复制到剪贴板，然后使用“复制”。

成功安装并启动服务后，请选择 *下一步*。

## 14.完成
“摘要”页面提供已完成任务的摘要列表。选择完成。`Ambari Web`将在您的Web浏览器中打开。
