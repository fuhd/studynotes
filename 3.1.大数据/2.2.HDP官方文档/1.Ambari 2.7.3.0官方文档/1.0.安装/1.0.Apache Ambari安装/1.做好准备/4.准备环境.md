...
#### 7.2.3.在MySQL中配置Druid和Superset元数据存储
**关于这个任务**

Druid和Superset需要关系数据存储来存储元数据。要使用MySQL，请安装MySQL并为Druid Metastore创
建数据库。
...
1. 启动MySQL监视器：
    ...
    mysql -u root -p
    ```
2. 为Druid和`Superset Metastore`创建数据库：
    ```sql
    CREATE DATABASE druid DEFAULT CHARACTER SET utf8;
    CREATE DATABASE superset DEFAULT CHARACTER SET utf8;
    ```
3. 创建druid和`superset`用户帐户，用您的密码替换最终的`IDENTIFIED BY`字符串：
    ```sql
    CREATE USER 'druid'@'%' IDENTIFIED BY '9oNio)ex1ndL';
    CREATE USER 'superset'@'%' IDENTIFIED BY '9oNio)ex1ndL';
    ```
4. 为德druid和`superset`帐户分配权限：
    ```sql
