...
+ 在 **2阶段** 时，如果Sentinel节点向 **1阶段** 选出来的从节点执行`slaveof no one`一直失败
    （例如该从节点此时出现故障），当此过程超过`failover-timeout`时，则故障转移失败。 
+ 在 **2阶段** 如果执行成功，Sentinel节点还会执行`info`命令来确认 **1阶段** 选出来的节点确实
    晋升为主节点，如果此过程执行时间超过`failover-timeout`时，则故障转移失败。
+ 如果 **3阶段** 执行时间超过了`failover-timeout`（不包含复制时间），则故障转移失败。
