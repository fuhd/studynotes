...
参考下面的示例，**第二个case是必要的，因为没有它的话，match表达式对于任何非BinOp的expr入参都会抛
出MatchError**。在本例中，**对于第二个case，我们并没有给出任何代码，因此如果这个case被运行，什么都
不会发生。两个case的结果都是unit值，即：()，这也是整个match表达式的结果**。
```scala
expr match {
