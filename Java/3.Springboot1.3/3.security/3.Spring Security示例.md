...
在方法级别进行授权，首先在配置上添加`@EnableGlobalMethodSecurity(prePostEnabled = true)`。
```java
@Configuration
...
```
如果`@PreAuthorize("expression")` 中，`expression`表达式结果为true，再调用被注解的方法。
`@PostAuthorize("expression")`中，先调用被注解的方式，然后再检查`expression`的值，不通过返回403。
